...
class GeneralizedBalancedSoftmaxLoss(nn.Module):
    def __init__(self, class_counts: torch.Tensor, tau: float = 1.0, weight: torch.Tensor | None = None) -> None:
        super().__init__()
        self.tau = tau
        self.weight = weight
        # Avoid log(0) by clamping counts to at least 1.0
        class_counts = class_counts.float()
        class_counts = torch.clamp(class_counts, min=1.0)
        # Store log_class_counts as a buffer so it moves with the model
        self.register_buffer("log_class_counts", torch.log(class_counts))

    def forward(self, logits: torch.Tensor, labels: torch.Tensor) -> torch.Tensor:
        # Logit adjustment: logits + tau * log(class_counts)
        # We only add this during training (if this loss is used in train mode)
        adjusted_logits = logits + self.tau * self.log_class_counts
...
def classification_loss(train_df: pd.DataFrame, label_to_idx: dict[str, int], args: argparse.Namespace, device: torch.device) -> nn.Module:
    ...
    if hasattr(args, "tau") and args.tau > 0.0:
        y = np.array([label_to_idx[label] for label in train_df["label"]])
        counts = np.bincount(y, minlength=len(label_to_idx))
        class_counts = torch.tensor(counts, dtype=torch.float32, device=device)
        return GeneralizedBalancedSoftmaxLoss(class_counts=class_counts, tau=args.tau, weight=weight)
